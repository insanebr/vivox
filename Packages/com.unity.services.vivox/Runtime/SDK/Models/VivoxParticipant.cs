...
namespace Unity.Services.Vivox
{
    ...
    {
        readonly IVivoxServiceInternal m_VivoxService;
        IParticipant m_ParentParticipant;
        GameObject m_ParticipantTapGameObject;

        internal VivoxParticipant(IVivoxServiceInternal vivoxService, IParticipant participant)
        {
            ...
            m_VivoxService.LoggedOut += DestroyTap;

            if (m_ParentParticipant.IsSelf)
            {
                m_VivoxService.UserInputDeviceMuteStateChanged += OnUserInputDeviceMuteStateChanged;
            }
        }
            ...
            if (m_ParentParticipant == null)
            {
                VivoxLogger.LogException(
                    new InvalidOperationException(
                        ...
                        $"If this is a cached reference, please update the reference when receiving an update from our VivoxService.Instance.{nameof(VivoxService.Instance.ParticipantRemovedFromChannel)} event."
                    )
                );
        ...
        /// </summary>
        public bool IsMuted => IsSelf ? m_VivoxService.Client.AudioInputDevices.Muted : (m_ParentParticipant.LocalMute || m_ParentParticipant.IsMutedForAll);
        /// <summary>
