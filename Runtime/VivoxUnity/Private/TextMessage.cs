...
namespace Unity.Services.Vivox
{
    ...
    internal class ChannelTextMessage : IChannelTextMessage
    {
        internal ChannelTextMessage(IChannelSession parent, JSONChannelMessage jsonChannelMessage)
        {
            ...
            Key = $"sip:.{jsonChannelMessage.Sender.Issuer}.{jsonChannelMessage.Sender.Name}.{environmentId}@{jsonChannelMessage.Sender.Domain}";
            Sender = new AccountId(Key, Key); // The second value of Key is only temporary until we pass in display name, for now Key is at least unique if not user friendly
