...
namespace Unity.Services.Vivox
{
    internal class LoginSession : ILoginSession
    {
        ...
        private void HandleAccountEditMessage(vx_evt_base_t eventMessage)
        {
            ...
            OnAccountEditMessage(new VivoxMessage(message));
        }

        protected virtual void OnAccountEditMessage(VivoxMessage e)
        {
            EventHandler<VivoxMessage> handler = DirectedMessageEdited;
        ...
        private void HandleAccountDeleteMessage(vx_evt_base_t eventMessage)
        {
            ...
            OnAccountDeleteMessage(new VivoxMessage(message));
        }

        protected virtual void OnAccountDeleteMessage(VivoxMessage e)
        {
            EventHandler<VivoxMessage> handler = DirectedMessageDeleted;
            handler?.Invoke(this, e);
        }
        ...
        private void HandleAccountArchiveMessage(vx_evt_base_t eventMessage)
        {
            ...
            _internalChatHistoryResults.AddOrUpdate(queryId,
                new List<VivoxMessage>() { new VivoxMessage(message) },
                (k, l) =>
                {
                    l.Add(new VivoxMessage(message));
                    return l;
                });
